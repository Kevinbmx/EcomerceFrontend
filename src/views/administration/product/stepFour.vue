  ...
  <div>
    <v-layout row>
      <v-flex xs12 sm12 md12>
        <v-alert v-if="mostrar" :value="true" type="info">
    ...
    <v-layout wrap>
      <v-flex xs12 sm6 md3 v-for="(file, index) in files" :key="index">
        <v-card>
          <v-img :src="file.url" :lazy-src="file.url" aspect-ratio="1" contain>
          </v-img>

          <v-card-actions>
            <v-btn text color="orange" @click="onPickFile(index)"
              >Upload Image</v-btn
            >
            <input
              type="file"
            ...
            <v-btn
              text
              color="orange"
              v-show="dessapearDeleteButton"
              @click="deleteFileImage(index)"
              >delete</v-btn
            >
          </v-card-actions>
        </v-card>
      ...
      <v-flex xs12 sm6 md3>
        <v-btn @click="addFileImage" :disabled="disableButtonAddFile"
          >otro</v-btn
        >
      </v-flex>
    </v-layout>
