<template>
  <div>
    <v-row v-if="hasDirection" class="mx-1">
      ...
      <v-col cols="12" sm="12" md="12">
        <v-btn block outlined color="secondary" @click="showFormDirection()">
          nueva direccion<v-icon right>add</v-icon>
        </v-btn>
      </v-col>
      <v-col
        cols="12"
        ...
        :key="index"
        :class="{ 'is-active': dir.id === selectedDirection }"
      >
        <v-row>
          ...
          <v-col cols="12" sm="12" md="12" class="">
            ...
            <gmap-map
              style="height: 300px"
              :zoom="14"
              :center="converToPosition(dir.latitud, dir.longitud)"
            >
              <gmap-marker
          ...
          <v-col cols="12" sm="12" md="12">
            <v-btn
              block
              color="secondary"
              @click="setSelectedDirection(dir.id)"
            >
              Seleccionar
            </v-btn>
          </v-col>
      ...
      <v-col cols="12" sm="12" md="12">
        <div v-if="showErrorSelection" class="theme--light error--text">
          <span>Seleccione una direccion antes de confirmar su pedido</span>
        </div>
      </v-col>
    </v-row>
    <v-row v-else>
      <v-col cols="12" sm="12" md="12">
        <v-btn block outlined color="secondary" @click="showFormDirection()">
          Direcciones guardas<v-icon>add</v-icon>
        </v-btn>
      </v-col>
      <v-col cols="12" sm="6" md="6">
        ...
        <div class="v-messages theme--light error--text">
          <span>{{ errors.first("direction") }}</span>
        </div>
        <v-text-field
      ...
      <v-col cols="12" sm="6" md="6">
        ...
        >
          <gmap-marker
            :position="
              converToPosition(new_direction.latitud, new_direction.longitud)
            "
            :clickable="false"
            :draggable="false"
          />
        </gmap-map>
        <div v-if="showMark" class="theme--light error--text">
          <span>marque su direccion en el mapa</span>
        </div>
      </v-col>
    </v-row>
    <v-row>
      <v-col cols="12" sm="12" md="12" class="align-rigth">
        <v-btn color="primary" @click="continuar()"> Siguiente </v-btn>
      </v-col>
    </v-row>
  </div>
</template>
...
export default {
  ...
  created() {
    this.$getLocation({}).then((coordenada) => {
  ...
  methods: {
    ...
    continuar() {
      if (!this.hasDirection) {
        if (
          this.new_direction.latitud == 0 &&
          this.new_direction.longitud == 0
        ) {
          this.showMark = true;
        } else {
          this.showMark = false;
