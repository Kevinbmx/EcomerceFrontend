...
const state = {
  ...
  //para insertar en la carpeta correcta de firebase
  firebaseCategoryfile: process.env.VUE_APP_FI_CATEGORY_FILE_URL
}
      ...
      if (state.image.state == 'update') {
        firebase.auth().signInWithEmailAndPassword(state.userFirebase, state.passFirebase)
          .then(() => {
            ...
            var desertRef = storageRef.child(state.firebaseCategoryfile + '/' + state.image.oldPathName);
    ...
    return new Promise((resolve) => {
      ...
      firebase.auth().signInWithEmailAndPassword(state.userFirebase, state.passFirebase)
        .then(() => {
          ...
          var uploadTask = storageRef.child(state.firebaseCategoryfile + '/' + uuid + ext).put(element.file[0], metadata);
