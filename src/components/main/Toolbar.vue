  ...
  <v-card tile dark class="primary">
    <v-row no-gutters class="pt-2 pb-2">
      <v-col
        ...
        order-md="1"
      >
        <router-link class="estiloTitulo" :to="{ name: 'mainPage' }"
          >ni&ntilde;o tienda</router-link
        >
      </v-col>
      <v-col
        cols="12"
      ...
      <v-col cols="8" sm="4" md="3" order="2" order-sm="3" order-md="3">
        ...
        >
          ...
          <v-btn text @click="goToCarrito()" :to="{ name: 'carrito' }">
            <label
              >carrito({{
                parseInt(this.$store.getters.getCantidadCarrito)
              }})</label
            >
    ...
    <v-navigation-drawer right temporary v-model="rightDrawer" fixed light>
      <v-list-item>
        <v-list-item-content>
...
export default {
  ...
  methods: {
    ...
    fillCategory() {
      this.$myApi.get(categoryParentUrl).then((response) => {
    ...
    getCarritoByPedidoId(pedido_id) {
      return new Promise((resolve) => {
        this.$myApi.get(carritoUrl + "/" + pedido_id).then((response) => {
    ...
    fillCarrito() {
      ...
      if (pedido_id == null && token != null) {
        this.$store.dispatch("selectPedidoByUserId").then((response) => {
        ...
        this.$store.dispatch("selectPedidoByUserId").then((response1) => {
          ...
          if (response1 !== null) {
            if (response1.id !== parseInt(pedido_id)) {
              ...
              let userCart;
              let oldCarrito;
              this.getCarritoByPedidoId(parseInt(response1.id)).then(
                (responseUser) => {
                  userCart = responseUser;
                  this.getCarritoByPedidoId(parseInt(pedido_id)).then(
                    (responseUser) => {
                      oldCarrito = responseUser;
                      //console.log('old carrito', oldCarrito)
                      //console.log('userCart ', userCart)
                      for (var index in oldCarrito) {
                        let find = userCart.find(
                          (file) =>
                            file.product_id !== oldCarrito[index].product_id
                        );
                        if (find) {
                          let objCart = {
                          ...
                          this.$store.dispatch(
                            "createAndUpdateCarrito",
                            objCart
                          );
                        }
                      }
                      this.$store.dispatch(
                        "deletePedidoAndCarritoByPedidoId",
                        oldCarrito[index].pedido_id
                      );
                      // .then(responseDelete=>{
            ...
            this.$store
              .dispatch("selectPedidoById", pedido_id)
              .then((responsePedido) => {
                this.$store
                  .dispatch("updatePedido", responsePedido.id)
                  .then((responseUpdatePedido) => {
                    if (responseUpdatePedido) {
                      this.$store.dispatch(
                        "selectCarritoByPedidoId",
                        responsePedido.id
                      );
                      // .then(response3 =>{
                      // //console.log('carrito ',response3)
        ...
        this.$store.dispatch("selectPedidoById", pedido_id).then((response) => {
